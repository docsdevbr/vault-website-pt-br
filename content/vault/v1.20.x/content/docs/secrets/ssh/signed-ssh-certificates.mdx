...
1.  Configure Vault with a CA for signing client keys using the `/config/ca`
    ...
    If you are using a managed key, specify the managed key name or ID:
    ```text
        $ vault write ssh-client-signer/config/ca \
        managed_key_name="..." \
    ```
    See the [Managed Keys](/vault/docs/enterprise/managed-keys) page for more information on managed keys.
