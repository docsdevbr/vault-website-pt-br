...
At a high level, the Secrets Store CSI driver lets you create `SecretProviderClass` objects.
This object defines which secret provider to use and what secrets to retrieve. When pods requesting CSI volumes
are created, the Secrets Store CSI driver sends the request to the Vault Secrets Store CSI provider if the provider
