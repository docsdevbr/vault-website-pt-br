...
<Tip title="Metrics from Leader vs Standby nodes">
  ...
  In most cases the active leader node telemetry is most useful, particularly
  after initial startup. Some metrics are **only** reported by the leader node,
  for example
  [active `vault.identity.*` metrics](/vault/docs/internals/telemetry/metrics/all#vault-identity-entity-active-monthly).
...
node. With unauthenticated metrics access enabled, the standby node responds
with its own metrics. With unauthenticated metrics access disabled:

- Vault Enterprise standby nodes try to service the request.
- Vault Community redirect the request to the leader node.
